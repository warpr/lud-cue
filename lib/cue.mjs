...
export function parseCommand(command, args, track) {
    switch (command) {
        ...
        case 'INDEX':
            ...
            indexes.push({ idx: parseInt(idx, 10), 'minutes-seconds-frames': startTime, seconds });
...
export function processSubTracks(records) {
    return records.reduce((memo, record) => {
        ...
        if (record.indexes.length === 1) {
            ...
            delete record.start.idx;
            delete record.indexes;
            memo.push(record);
            return memo;
        }

        const indexes = record.indexes.filter(i => i.idx > 0);
